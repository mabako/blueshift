...
local gasStation = 
{
	...
	{ 276.3710, 1413.0576, 10.4359, "to the Oil Refinery, Bone County"},
...
function createDepotMarker( )
	...
	depotMarker = createMarker( 984.5029, -898.5253, 42.4193, "checkpoint", 2, 0, 255, 0, 200 )
	depotBlip = createBlip( 984.5029, -898.5253, 42.4193, 0, 2, 0, 255, 0, 255, 0, 99999.0 )
end
			...
			if ( source == stationMarker ) then
				...
				setTimer(
					function ( )
						...
						local money = math.floor( ( getDistanceBetweenPoints3D( 984.5029, -898.5253, 42.4193, x, y, z ) )/10 )
